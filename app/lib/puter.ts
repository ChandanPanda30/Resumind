...
interface PuterStore {
  ...
  auth: {
    ...
    googleLoginSuccess: (credential: string) => void;
...
const isTokenExpired = (token: string) => {
  try {
    ...
    if (!decoded.exp) return false; // no expiry field means unknown
...
export const usePuterStore = create<PuterStore>((set, get) => {
  ...
  const googleLoginSuccess = (credential: string) => {
    if (isTokenExpired(credential)) {
      setError("Google token has expired");
      return;
    }
    try {
      const user: GoogleUser = jwtDecode(credential);
      ...
      setError("Failed to decode Google credentials");
    }
  };

  const googleLoginError = () => setError("Google Sign-in failed");
  ...
  const signOut = () => {
    ...
    // Best-effort cleanup local state, keeping cloud files intact for persistence
    void (async () => {
  ...
  const init = () => {
    const puter = getPuter();
    if (puter) set({ puterReady: true });

    checkAuthStatus(); // restore auth state
